...
function fallbackMotionForThreat(threat) {
  ...
  const explicitSpeed = Number(
    threat?.velocity?.speedKmh ?? threat?.speedKmh ?? threat?.speed_kmh,
  );
  const explicitBearing = Number(
...
function predictedThreatPosition(threat, nowMs = Date.now()) {
  ...
  if (
    ...
    window.NEPTUN &&
    typeof window.NEPTUN.predict === "function"
  ) {
    try {
  ...
  if (
    !Number.isFinite(motion.speedKmh) ||
    motion.speedKmh <= 0 ||
  ...
  const elapsedMinutes = Math.max(
    0,
    Math.min((nowMs - anchor) / 60000, motion.maxMinutes),
  );
...
function showThreatPopup(threat, marker) {
  ...
  const updated = threat.updatedAt
    ? new Date(threat.updatedAt).toLocaleTimeString("uk-UA", {
...
let syncThreatMarkers = function () {
  const active = currentThreats.filter(
    (t) =>
      ...
      Number.isFinite(Number(t.lon)),
  );
  const activeIds = new Set();

  for (const threat of active) {
    const id = String(
      threat.id || `${threat.type}-${threat.lat}-${threat.lon}`,
    );
    activeIds.add(id);
...
function startNeptunThreats() {
  ...
  if (window.NEPTUN && typeof window.NEPTUN.RealtimeClient === "function") {
    try {
      ...
      neptunRealtimeClient = new window.NEPTUN.RealtimeClient(
        "https://neptun.in.ua",
      );
...
function applyNeptunAlerts(payload = {}) {
  ...
  const oblastFeatures = (neptunOblastsGeoJSON?.features || []).filter(
    (feature) => featureMatchesItems(feature, activeOblasts),
  );
...
async function fetchJSON(url) {
  if (IS_NATIVE_APP) {
    ...
    const text = await nativeGet(NEPTUN_BASE + target, {
...
async function fetchTelegramItems() {
  if (IS_NATIVE_APP) {
    const page = await nativeGet("https://t.me/s/" + TG_CHANNEL, {
...
async function loadNeptunBoundaries() {
  ...
  if (
    oblasts?.type !== "FeatureCollection" ||
    !Array.isArray(oblasts.features)
  ) {
...
function addNeptunAlertLayers() {
  map.addSource("neptun-alert-oblasts", {
    type: "geojson",
    data: emptyFeatureCollection(),
  });
  map.addSource("neptun-alert-raions", {
    type: "geojson",
    data: emptyFeatureCollection(),
  });
...
const OBLAST_LABELS = {
  ...
  features: [
    ...
    ["Закарпатська область", 23.2, 48.4],
    ...
    ["Херсонська область", 34.08, 46.7],
    ["Запорізька область", 35.72, 47.2],
    ...
    try {
      ...
      if (data?.type === "FeatureCollection" && data.features?.length >= 24) {
        data.features.forEach((feature, index) => {
          ...
          feature.properties.neptunKey =
            feature.properties.neptunKey || REGION_NEPTUN_KEYS[index];
        });
        return data;
      }
    } catch (_) {}
  ...
  for (const base of REGION_BASES) {
    try {
      ...
      if (features.length < 24)
        throw new Error(`Отримано лише ${features.length} областей`);

      const result = { type: "FeatureCollection", features };
      try {
        localStorage.setItem(
          "ukraine-oblasts-geojson-v3",
...
function addRegionLayers(regionsData) {
  ...
  map.on("mousemove", "oblast-fill", (event) => {
    ...
    if (hoveredRegionId !== null) {
      map.setFeatureState(
        { source: "ukraine-regions", id: hoveredRegionId },
        { hover: false },
      );
    }
...
["dragstart", "zoomstart", "rotatestart", "pitchstart"].forEach((name) =>
  map.on(name, () => clearTimeout(returnTimer)),
);
["dragend", "zoomend", "rotateend", "pitchend"].forEach((name) =>
  map.on(name, startReturnTimer),
);
...
document
  ...
  .addEventListener("click", () => returnToUkraine(true));
document
...
async function loadTelegramNews() {
  ...
  try {
    ...
    telegramFeedStatus.textContent =
      "ОШИБКА: " + String((error && error.message) || error).slice(0, 80);
  } finally {
    telegramBusy = false;
  }
}
...
function setTelegramFeedHidden(hidden) {
  ...
  try {
    localStorage.setItem("telegram-feed-hidden", hidden ? "1" : "0");
  } catch (_) {}
}

telegramFeedToggle?.addEventListener("click", () =>
  ...
  for (const m of mutations) {
    if (m.type === "characterData") applyWordReplacements(m.target.parentNode);
      ...
      m.addedNodes.forEach((n) =>
        applyWordReplacements(n.nodeType === 1 ? n : n.parentNode),
      );
  }
...
(function initSecondFeed() {
  ...
  async function loadFeed2() {
    ...
    try {
      ...
      const page = await nativeGet("https://t.me/s/" + FEED2_CHANNEL, {
        "Accept-Language": "uk-UA,uk;q=0.9,en;q=0.7",
      });
...
(function initPostTracking() {
  ...
  const TYPE_RULES = [
    ...
    [
      "ballistic",
      new RegExp(`(?<![${NC}])(баліст|баллист|іскандер|искандер)`, "i"),
    ],
    ...
    [
      "uav",
      new RegExp(
        `(?<![${NC}])(шахед|шахід|шахид|герань|бпла|бпак|дрон|ударн)`,
        "i",
      ),
    ],
  ];
  ...
  const detectType = (s) => {
    for (const [t, re] of TYPE_RULES) if (re.test(s)) return t;
    return null;
  };
  ...
  function parsePostLine(line) {
    ...
    if (place.length < 2 || place.length > 40 || place.split(" ").length > 3)
      return null;
    if (
      !/^[А-ЯІЇЄҐЁ]/.test(place) ||
      STOP.has(place.toLowerCase()) ||
      /област|район|громад/i.test(place)
    )
      return null;
  ...
  function replay(items) {
    ...
    for (const item of items) {
      ...
      for (const c of parsePostText(item.text)) {
        ...
        if (c.op === "+")
          state.set(np + "|" + c.type, { ...c, ts, url: item.url });
        else
  ...
  let geoCache = {};
  try {
    geoCache = JSON.parse(localStorage.getItem(CACHE_KEY) || "{}");
  } catch (e) {
    geoCache = {};
  }
  const saveCache = () => {
  ...
  function cached(key) {
    ...
    return Date.now() - hit.ts < 6 * 3600 * 1000 ? null : undefined;
  }
  ...
  async function nominatim(name) {
    const url =
      "https://nominatim.openstreetmap.org/search?format=jsonv2&limit=5&accept-language=uk" +
      "&countrycodes=ua&bounded=1&viewbox=" +
      POST_VIEWBOX +
      "&q=" +
      encodeURIComponent(name);
    const text = IS_NATIVE_APP
      ? await nativeGet(url, {
          "User-Agent": "ChysteNebo-personal-map/1.0",
          "Accept-Language": "uk",
        })
  ...
  function geocode(place) {
    if (POST_OVERRIDES[place])
      return Promise.resolve({
        lat: POST_OVERRIDES[place][0],
        lon: POST_OVERRIDES[place][1],
      });
  ...
  syncThreatMarkers = function () {
    try {
      currentThreats = currentThreats
        .filter((t) => !(t && t.pt))
        .concat(postThreats.filter(isFresh));
    } catch (e) {
      console.warn("Post merge error:", e);
    }
  ...
  async function poll() {
    ...
    busy = true;
    try {
      const page = await nativeGet("https://t.me/s/" + POST_CHANNEL, {
        "Accept-Language": "uk-UA,uk;q=0.9,en;q=0.7",
      });
      const alive = [...replay(parseTelegramPage(page)).entries()].filter(
        ([, c]) => Date.now() - c.ts < POST_TTL_MIN * 60000,
      );
      const next = [];
      ...
      console.warn("Post tracking error:", e);
    } finally {
      busy = false;
      const sig = JSON.stringify(
        postThreats
          .filter(isFresh)
          .map((t) => [t.id, t.lat, t.lon, t.count, t.updatedAt]),
      );
      if (sig !== lastSig) {
        lastSig = sig;
        refresh();
